...
- An MCP server at `/api/v1/mcp` exposes the same agent as the `ask_neuroapi` tool over Streamable HTTP, for AI clients and MCP-native code.
...
| [07_mcp_client.py](examples/07_mcp_client.py) | Call the agent over MCP: connect to the Streamable HTTP server with the official MCP SDK and run the `ask_neuroapi` tool. |
